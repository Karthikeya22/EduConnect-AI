...
const GradePredictor: React.FC<GradePredictorProps> = (props) => {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'platform_activity_logs' }, (payload) => {
                ...
                // Dynamic class health based on sentiment
                setClassHealth(prev => {
                    const healthAdjustment = newLog.action_type === 'submission' ? 0.5 : -0.1;
                    return Math.min(100, Math.max(0, prev + healthAdjustment));
                });
    ...
    const fetchStudents = async () => {
        ...
        try {
            const [
                ...
                supabase.from('student_assignment_logs').select('*'),
                supabase.from('platform_activity_logs').select('user_id, created_at').order('created_at', { ascending: false })
            ]);

            const studsList = studentsData || [];
            const subs = submissions || [];
            const acts = activityLogs || [];

            if (studsList.length === 0) {
                setStudents([]);
                return;
            }

            const profiles: StudentRiskProfile[] = studsList.map((s: any) => {
                const mySubs = subs.filter(sub => sub.student_id === s.id);
                const myActs = acts.filter(a => a.user_id === s.id);

                const gradedSubs = mySubs.filter(sub => sub.grade !== null && sub.grade !== undefined);
                const avgGrade = gradedSubs.length > 0 ? gradedSubs.reduce((acc, curr) => acc + curr.grade, 0) / gradedSubs.length : 100;

                const attendanceScore = Math.min(100, (myActs.length > 0 ? 50 : 20) + (myActs.length * 2));
                const lastActiveStr = myActs.length > 0 ? new Date(myActs[0].created_at).toLocaleDateString() : 'Never';

                let calculatedRisk = 100 - avgGrade;
                if (attendanceScore < 50) calculatedRisk += 20;
                if (mySubs.length === 0 && myActs.length === 0) calculatedRisk = 85;

                calculatedRisk = Math.min(100, Math.max(0, calculatedRisk));

                return {
                    id: s.id,
                    name: s.student_name || s.student_email?.split('@')[0] || 'Unknown',
                    email: s.student_email || s.email || '',
                    riskScore: Math.round(calculatedRisk),
                    attendance: Math.round(attendanceScore),
                    lastActive: lastActiveStr,
                    predictedGrade: calculateGrade(calculatedRisk),
                    trend: (calculatedRisk > 60 ? 'down' : (calculatedRisk < 30 ? 'up' : 'stable')) as any,
                    riskFactors: generateRiskFactors(calculatedRisk),
                    sentiment: Math.max(0, 100 - calculatedRisk - 5), // Deterministic sentiment
                    interventionCount: 0
                };
            });
    ...
    const generateRiskFactors = (risk: number) => {
        ...
        if (risk > 40 && risk % 2 !== 0) factors.push('Negative Sentiment Trend');
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            data: [
                                ...
                                Math.max(0, 100 - selectedStudent.riskScore - 15) // deterministic generic metric
                            ],
            ...
            <main ref={mainRef} className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
                ...
                <header className="h-20 bg-[var(--bg-card)] border-b-2 border-[var(--border-primary)] flex items-center justify-between px-8 shrink-0 z-20 transition-colors">
                    <div className="flex items-center space-x-4">
                        <h1 className="text-2xl font-black text-zinc-900 dark:text-white tracking-tighter uppercase font-['Space_Grotesk']">Predictor</h1>
                        <div className="h-6 w-px bg-zinc-200 dark:bg-white/10"></div>
                        <span className="text-xs font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest">Sentinel Hub</span>
                    </div>
                    <div className="flex items-center space-x-4">
                        <ThemeToggle />
                        <div className="hidden md:flex items-center space-x-2 px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-full border border-emerald-100 dark:border-emerald-800/30">
