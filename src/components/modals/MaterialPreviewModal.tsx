...
import { UniversalPreviewer } from '@/src/components/ui/UniversalPreviewer';
...
const MaterialPreviewModal: React.FC<MaterialPreviewModalProps> = ({ material, onClose }) => {
   ...
   return (
      <div ref={overlayRef} className="fixed inset-0 z-[3000] bg-[var(--text-primary)]/80 backdrop-blur-xl flex items-center justify-center p-6" onClick={handleClose}>
         <div ref={modalRef} className="w-full max-w-6xl bg-[var(--bg-card)] rounded-[2.5rem] shadow-[var(--shadow-xl)] overflow-hidden flex flex-col h-[90vh] border-2 border-[var(--border-primary)]" onClick={e => e.stopPropagation()}>

            <header className="p-8 md:p-10 border-b-2 border-[var(--border-primary)] bg-[var(--bg-card)] flex flex-col md:flex-row md:items-center justify-between gap-6 shrink-0">
            ...
            <div className="bg-[var(--bg-nested)] p-4 border-b-2 border-[var(--border-primary)] flex justify-center space-x-2 shrink-0">
               <button
            ...
            <div className="flex-1 relative overflow-hidden bg-zinc-100">
               {activeTab === 'content' ? (
                  <UniversalPreviewer 
                     url={material.location_url} 
                     filename={`${material.title}.${material.file_type}`}
                     onDownload={() => window.open(material.location_url, '_blank')}
                  />
               ) : (
