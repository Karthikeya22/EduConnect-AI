...
const AppSidebar: React.FC<AppSidebarProps> = ({ role, onNavigateTo, collapsed, setCollapsed, onLogout }) => {
  ...
  const teacherItems = [
    ...
    { Icon: Icons.IconChart, label: "Class Ledger", id: 'teacher-grades', path: '/teacher/grades' },
    ...
    { Icon: Icons.IconTrending, label: "Clickstream Hub", id: 'teacher-clickstream', path: '/teacher/clickstream' },
    { Icon: Icons.IconTarget, label: "Canvas Sync", id: 'teacher-canvas-sync', path: '/teacher/canvas-sync' },
  ];
  ...
  const studentItems = [
    ...
    { Icon: Icons.IconLab, label: "Laboratory", id: 'student-lab', path: '/student/lab' },
    ...
    <aside className={`bg-[var(--bg-card)] border-r-2 border-[var(--border-primary)] shadow-[var(--shadow-xl)] transition-all duration-500 flex flex-col z-[100] relative h-full shrink-0 ${collapsed ? 'w-[100px]' : 'w-[280px]'}`}>
          ...
          return (
            ...
            >
              <span
                ...
                className={`shrink-0 inline-block transition-colors ${active ? 'text-white' : 'text-[var(--text-muted)] group-hover:text-indigo-600 dark:group-hover:text-zinc-300'
                  }`}
              >
      ...
      <div className="mt-auto border-t border-zinc-100 dark:border-white/5 p-4 flex flex-col gap-2">
        ...
        <button
          ...
          className="w-full h-12 flex items-center justify-center text-[var(--text-muted)] hover:text-zinc-900 dark:hover:text-white bg-zinc-50 dark:bg-white/5 hover:bg-zinc-100 dark:hover:bg-white/10 rounded-xl transition-colors font-medium text-sm"
        >
