...
const StudentDiscussion: React.FC<StudentDiscussionProps> = ({ assignmentId, onBack }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const formattedPosts: Post[] = (logs || []).map(l => ({
        ...
        content: l.submission_content || '',
        timestamp: new Date(l.timestamp).toLocaleString(),
        parent_id: undefined // no metadata column to store parent_id
      }));
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      // Check if this is the first post for this user on this assignment
      ...
      const isFirstSubmission = !existing || existing.length === 0;

      await supabase.from('student_assignment_logs').insert({
        student_id: session.user.id,
        assignment_id: assignmentId,
        course_id: 'BIG_DATA_2026',
        interaction_type: isFirstSubmission ? 'submission' : 'discussion_post',
        submission_content: userPost,
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header className="h-20 bg-white dark:bg-[#0B1120] border-b border-zinc-200 dark:border-white/5 flex items-center justify-between px-8 shrink-0 z-20 transition-colors">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-xl font-black text-zinc-900 dark:text-white tracking-tighter uppercase font-['Space_Grotesk']">Discussion Hub</h1>
          </div>
          <div className="flex items-center space-x-4">
            <ThemeToggle />
            <div className="hidden md:flex items-center space-x-2 px-3 py-1.5 bg-cyan-50 dark:bg-cyan-900/20 text-cyan-600 dark:text-cyan-400 rounded-full border border-cyan-100 dark:border-cyan-800/30">
        ...
        <div className="flex-1 overflow-y-auto p-8 md:p-12 scroll-smooth bg-[#F8FAFC] dark:bg-[#020617] transition-colors">
        ...
        <div className="p-10 border-t bg-white shrink-0">
          <div className="relative">
            ...
            <button
              ...
              className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-[#0F172A] text-white rounded-xl flex items-center justify-center"
            >
