...
const StudentProgress: React.FC<StudentProgressProps> = ({ onBack, onNavigateTo, onLogout, onOpenNotifs, currentPath = 'student-progress' }) => {
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] dark:bg-[#020617] overflow-hidden relative font-['Plus_Jakarta_Sans'] transition-colors">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        <header className="h-20 bg-white dark:bg-[#0B1120] border-b border-zinc-100 dark:border-white/5 flex items-center justify-between px-12 shrink-0 z-40 transition-colors">
          <div className="flex items-center space-x-6">
            <button onClick={onBack} className="w-10 h-10 rounded-full hover:bg-zinc-50 dark:hover:bg-white/5 flex items-center justify-center text-zinc-400 transition-colors">←</button>
            <h1 className="text-2xl font-black text-zinc-900 dark:text-white tracking-tighter uppercase font-['Space_Grotesk']">Learning Ledger</h1>
          </div>
          <div className="flex items-center space-x-8">
            <ThemeToggle />
            <div className="hidden md:flex items-center space-x-2 bg-zinc-50 dark:bg-white/5 px-4 py-2 rounded-full border border-zinc-100 dark:border-white/5">
              <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span>
              <span className="text-[9px] font-black text-zinc-400 uppercase tracking-widest leading-none">Sync: {new Date().toLocaleTimeString()}</span>
            </div>
          </div>
        </header>

        <main className="flex-1 overflow-y-auto p-12 bg-[#F8FAFC] dark:bg-[#020617] scroll-smooth transition-colors">
          {loading ? (
            <ProgressSkeleton />
          ) : (
                    ...
                    <div key={i} className="p-8 hover:bg-zinc-50 transition-colors flex flex-col md:flex-row md:items-center justify-between gap-6">
                      ...
                      <div className="flex items-center gap-8">
                        <div className="text-right">
                          ...
                          <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${a.submission && a.submission.grade !== undefined && a.submission.grade !== null ? 'bg-green-100 text-green-600' : a.submission ? 'bg-yellow-100 text-yellow-600' : 'bg-zinc-100 text-zinc-400'}`}>
                            {a.submission ? (a.submission.grade !== undefined && a.submission.grade !== null ? `Score: ${a.submission.grade}` : 'Submitted') : 'Pending'}
                          </span>
                        </div>
