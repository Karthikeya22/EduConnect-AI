...
const AssetHub: React.FC<AssetHubProps> = ({ onBack, onLogout, onNavigateTo, currentPath, role = 'teacher' }) => {
    ...
    const fetchItems = useCallback(async (isSilent = false) => {
        ...
        try {
            ...
            if (role === 'student') {
                enriched = enriched.filter(i => i.is_visible !== false);
            }
            setItems(enriched);
        } catch (err: any) {
            console.error(err);
    ...
    const allFolders = Array.from(new Set(items.map(i => i.topic).filter((t): t is string => typeof t === 'string' && t.trim() !== ''))).sort() as string[];
    ...
    const handleRenameFolder = async (oldName: string) => {
        if (!renameValue.trim() || renameValue.trim().toLowerCase() === oldName.toLowerCase()) {
            setRenamingFolder(null);
            return;
        }
        const newName = renameValue.trim();
        if (allFolders.some(f => f.toLowerCase() === newName.toLowerCase())) {
            showToast("A folder with this name already exists.", 'error');
            return;
        }
        try {
            const { error } = await supabase.from('instructional_materials').update({ topic: newName }).eq('topic', oldName);
            if (error) throw error;
    ...
    const performUpload = async (files: FileList, fixedTopic: string | null, desc: string, isFolderUpload = false) => {
        try {
            const uploadPromises = Array.from(files).map(async (f: any) => {
                ...
                // Leave finalTopic as empty string to avoid NOT NULL constraints
                if (!finalTopic) finalTopic = "";
    ...
    return (
        <div className="flex h-screen bg-white dark:bg-[#020617] font-['Plus_Jakarta_Sans'] overflow-hidden transition-colors">
            <AppSidebar role={role} currentPath={currentPath} onNavigateTo={onNavigateTo} collapsed={sidebarCollapsed} setCollapsed={setSidebarCollapsed} onLogout={onLogout} />

            <main className="flex-1 flex flex-col min-w-0 h-full">
                {/* Header */}
                <header className="h-20 border-b border-[var(--border-primary)] px-8 flex items-center justify-between shrink-0 bg-[var(--bg-card)]">
                    <h1 className="text-2xl font-black text-[var(--text-primary)] tracking-tight">{role === 'teacher' ? 'Management Hub' : 'Asset Repository'}</h1>
                    <div className="flex items-center space-x-4">
                        <ThemeToggle />
                        {role === 'teacher' && (
                            <button className="flex items-center space-x-2 text-zinc-500 hover:text-zinc-900 dark:hover:text-white px-4 py-2 rounded-lg hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors">
                                <span className="text-sm font-bold">View as Student</span>
                            </button>
                        )}
                ...
                {/* Toolbar */}
                <div className="px-8 py-6 space-y-6 shrink-0 bg-[var(--bg-main)]">
                    <div className="flex flex-wrap items-center justify-between gap-4">
                        <div className="flex items-center space-x-3">
                            <button onClick={() => setCurrentFolder(null)} className={`px-4 py-2 border border-[var(--border-strong)] text-[var(--text-secondary)] font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-[var(--bg-nested)] transition-all shadow-sm ${!currentFolder ? 'bg-[var(--bg-nested)]' : 'bg-[var(--bg-card)]'}`}>All Assets</button>
                            {role === 'teacher' && !currentFolder && (
                                <button onClick={startCreateFolder} className="px-4 py-2 border border-zinc-200 dark:border-white/10 text-zinc-600 dark:text-zinc-300 font-bold text-xs rounded-md bg-white dark:bg-white/5 hover:bg-zinc-50 dark:hover:bg-white/10 transition-colors shadow-sm flex items-center space-x-2">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="relative w-full max-w-2xl">
                            ...
                            <input
                                ...
                                className="w-full h-10 pl-10 pr-4 border border-zinc-300 dark:border-white/10 dark:bg-[#0B1120] rounded-md text-sm text-zinc-700 dark:text-zinc-200 placeholder-zinc-400 focus:outline-none focus:border-zinc-500 focus:ring-1 focus:ring-zinc-500 transition-all"
                            />
                        </div>
                        <button className="ml-4 px-6 py-2 border border-zinc-300 dark:border-white/10 rounded-md text-sm font-bold text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-white/5">Search</button>
                    </div>

                    <div className="flex items-center justify-between text-sm text-zinc-500 border-b border-zinc-200 dark:border-white/10 pb-2">
                        <div className="flex items-center space-x-2">
                            <button onClick={() => setCurrentFolder(null)} className="font-bold text-zinc-900 dark:text-white hover:underline">[Online Version] EME6356.001S26.14525 Big Data & Learning Analytics</button>
                            {currentFolder && (
                                <>
                                    <span>/</span>
                                    <span className="font-bold text-zinc-700 dark:text-zinc-300">{currentFolder}</span>
                                </>
                            )}
                        </div>
                        <div className="flex items-center space-x-3">
                ...
                {/* File Table */}
                <div className="flex-1 overflow-auto px-8 pb-8 bg-[var(--bg-main)]" onClick={() => setShowUploadMenu(false)}>
                    <table className="w-full min-w-[800px] border-separate border-spacing-y-2">
                        <thead className="sticky top-0 bg-[var(--bg-main)] z-10 transition-colors">
                            <tr className="bg-[var(--bg-nested)] border border-[var(--border-primary)]">
                                <th className="w-12 py-4 text-left pl-6 rounded-l-2xl"><input type="checkbox" onChange={handleSelectAll} className="w-4 h-4 rounded border-[var(--border-strong)] text-[var(--brand-primary)]" /></th>
                                <th className="py-4 text-left text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em] w-12">Type</th>
                                <th className="py-4 text-left text-[10px] font-black text-[var(--brand-primary)] uppercase tracking-[0.2em] cursor-pointer">Resource Name ▲</th>
                                <th className="py-4 text-left text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em]">Created</th>
                                <th className="py-4 text-left text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em]">Modified</th>
                                <th className="py-4 text-left text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em]">By</th>
                                <th className="py-4 text-left text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em]">Size</th>
                                <th className="py-4 text-center text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em]">Status</th>
                                <th className="py-4 text-right text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em] pr-6 rounded-r-2xl">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-zinc-100 dark:divide-white/5">
                            {/* Back Button inside Folder */}
                            {currentFolder && (
                                <tr className="group hover:bg-zinc-50 dark:hover:bg-white/5 cursor-pointer" onClick={() => setCurrentFolder(null)}>
                                    <td className="py-4 pl-2"></td>
                                    <td className="py-4 text-center text-zinc-400 font-bold">..</td>
                                    <td className="py-4 font-bold text-sm text-zinc-500 dark:text-zinc-400" colSpan={7}>Back to Root</td>
                                </tr>
                            )}
                            ...
                            {isCreatingFolder && !currentFolder && (
                                <tr className="group bg-blue-50/20 dark:bg-blue-900/10">
                                    <td className="py-4 pl-2"><input type="checkbox" disabled className="w-4 h-4 rounded border-zinc-300 dark:border-white/10" /></td>
                                    ...
                                    <td className="py-4">
                                        <input
                                            ...
                                            className="bg-white dark:bg-[#0B1120] border border-blue-400 dark:border-blue-500/50 rounded px-2 py-1 text-sm font-bold text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-blue-200 w-64"
                                            autoFocus
                                        />
                                    </td>
                            ...
                            {displayedFolders.map(folderName => {
                                ...
                                return (
                                    <tr key={folderName} className={`ui-card hover:bg-[var(--bg-nested)] transition-all ${selectedIds.has(folderName) ? 'ring-2 ring-[var(--brand-primary)]' : ''} ${isFolderHidden ? 'opacity-60' : ''}`}>
                                        <td className="py-6 pl-6 rounded-l-[2.5rem]"><input type="checkbox" checked={selectedIds.has(folderName)} onChange={() => handleSelection(folderName, true)} className="w-4 h-4 rounded border-[var(--border-strong)] text-[var(--brand-primary)]" /></td>
                                        <td className="py-4">
                                            <div className="relative">
                                                <IconFolder />
                                                {isFolderHidden && <div className="absolute -top-1 -right-1 w-2 h-2 bg-zinc-400 rounded-full border border-white dark:border-[#020617]" />}
                                            </div>
                                        </td>
                                        <td className="py-4 font-bold text-sm text-zinc-900 dark:text-zinc-100">
                                            {renamingFolder === folderName ? (
                                                <input
                                                    ...
                                                    autoFocus
                                                    className="bg-zinc-50 dark:bg-white/5 border border-blue-400 dark:border-blue-500/50 rounded px-2 py-1 text-sm font-bold text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-blue-200 w-64"
                                                    onClick={(e) => e.stopPropagation()}
                                                />
                                            ) : (
                                                <span className="cursor-pointer hover:underline hover:text-[#0E6C50]" onClick={() => setCurrentFolder(folderName)}>
                                                    {folderName}
                                                </span>
                                            )}
                                            {isFolderHidden && <span className="ml-2 text-[9px] uppercase tracking-widest text-zinc-400 bg-zinc-100 dark:bg-white/10 px-2 py-0.5 rounded-full">Hidden</span>}
                                        </td>
                                        ...
                                        <td className="py-4 text-right pr-2 relative">
                                            ...
                                            {activeMenu === folderName && (
                                                <div className="absolute right-8 top-8 w-44 bg-white dark:bg-[#0B1120] rounded-xl shadow-xl border border-zinc-100 dark:border-white/10 z-50 overflow-hidden animate-fade-in origin-top-right">
                                                    {role === 'teacher' ? (
                                                        <>
                                                            <button onClick={(e) => { e.stopPropagation(); setRenamingFolder(folderName); setRenameValue(folderName); setActiveMenu(null); }} className="w-full text-left px-4 py-3 text-xs font-bold text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-white/5 flex items-center space-x-2">
                                                                <IconEdit /> <span>Rename</span>
                                                            </button>
                                                            <button onClick={(e) => { e.stopPropagation(); handleToggleVisibility(folderName, true, !isFolderHidden); }} className="w-full text-left px-4 py-3 text-xs font-bold text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-white/5 flex items-center space-x-2 border-t border-zinc-50 dark:border-white/5">
                                                                {isFolderHidden ? <><IconEye /> <span>Show to Students</span></> : <><IconEyeOff /> <span>Hide from Students</span></>}
                                                            </button>
                                                            <button onClick={(e) => { e.stopPropagation(); handleSingleDelete(folderName, true); }} className="w-full text-left px-4 py-3 text-xs font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 flex items-center space-x-2 border-t border-zinc-50 dark:border-white/5">
                                                                <IconTrash /> <span>Delete Folder</span>
                                                            </button>
                                                        </>
                                                    ) : (
                                                        <button onClick={(e) => { e.stopPropagation(); setActiveMenu(null); }} className="w-full text-left px-4 py-3 text-xs font-bold text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-white/5 flex items-center space-x-2">
                            ...
                            {displayedFiles.map(item => (
                                <tr key={item.id} className={`ui-card hover:bg-[var(--bg-nested)] transition-all ${selectedIds.has(item.id) ? 'ring-2 ring-[var(--brand-primary)] bg-[var(--bg-nested)]' : ''} ${item.is_visible === false ? 'opacity-60 bg-[var(--bg-nested)]' : ''}`}>
                                    <td className="py-6 pl-6 rounded-l-[2.5rem]"><input type="checkbox" checked={selectedIds.has(item.id)} onChange={() => handleSelection(item.id, true)} className="w-4 h-4 rounded border-[var(--border-strong)] text-[var(--brand-primary)]" /></td>
                                    <td className="py-4">
                                        <div className="relative">
                                            <IconFile type={item.file_type} />
                                            {item.is_visible === false && <div className="absolute -top-1 -right-1 w-2 h-2 bg-zinc-400 rounded-full border border-white dark:border-[#020617]" />}
                                        </div>
                                    </td>
                                    <td className="py-4">
                                        <div className="flex items-center">
                                            <a href={item.location_url} target="_blank" rel="noreferrer" className="font-bold text-sm text-zinc-900 dark:text-zinc-100 hover:underline hover:text-[#0E6C50] block truncate max-w-[250px]">{item.title}</a>
                                            {item.is_visible === false && <span className="ml-2 text-[9px] uppercase tracking-widest text-zinc-400 bg-zinc-100 dark:bg-white/10 px-2 py-0.5 rounded-full">Hidden</span>}
                                        </div>
                                    </td>
                                    ...
                                    <td className="py-4 text-right pr-2 relative">
                                        ...
                                        {activeMenu === item.id && (
                                            <div className="absolute right-8 top-8 w-44 bg-white dark:bg-[#0B1120] rounded-xl shadow-xl border border-zinc-100 dark:border-white/10 z-50 overflow-hidden animate-fade-in origin-top-right">
                                                <button onClick={(e) => { e.stopPropagation(); window.open(item.location_url, '_blank'); setActiveMenu(null); }} className="w-full text-left px-4 py-3 text-xs font-bold text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-white/5 flex items-center space-x-2">
                                                    <IconDownload /> <span>Download</span>
                                                </button>
                                                {role === 'teacher' && (
                                                    <>
                                                        <button onClick={(e) => { e.stopPropagation(); handleToggleVisibility(item.id, false, !!item.is_visible); }} className="w-full text-left px-4 py-3 text-xs font-bold text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-white/5 flex items-center space-x-2 border-t border-zinc-50 dark:border-white/5">
                                                            {item.is_visible === false ? <><IconEye /> <span>Show to Students</span></> : <><IconEyeOff /> <span>Hide from Students</span></>}
                                                        </button>
                                                        <button onClick={(e) => { e.stopPropagation(); handleSingleDelete(item.id, false); }} className="w-full text-left px-4 py-3 text-xs font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 border-t border-zinc-50 dark:border-white/5 flex items-center space-x-2">
